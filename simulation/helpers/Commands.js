...
var g_Commands = {
	...
	"delete-entities": function(player, cmd, data)
	{
		for (const ent of data.entities)
		{
			if (!data.controlAllUnits)
			{
				...
				if (cmpIdentity &&
					cmpIdentity.HasClass("BattalionLeader")
					 && cmpIdentity.IsUndeletable())
					continue;
	...
	{
		const entities = data.entities.filter(ent =>
			GetBattalionLeaderEntity(ent) == INVALID_ENTITY);

		GetFormationUnitAIs(entities, player, cmd, data.formation, true).forEach(cmpUnitAI =>
		{
	...
	"promote": function(player, cmd, data)
	{
		...
		for (const ent of data.entities)
		{
			...
			if (cmpLeader)
			{
				cmpLeader.AddExperience(cmpLeader.GetRequiredXp());
				continue;
			}
...
function ProcessCommand(player, cmd)
{
	...
	if (data.entities)
		data.entities = CollapseBattalionEntities(data.entities);
...
function GetBattalionLeaderEntity(ent)
{
	if (Engine.QueryInterface(ent, IID_BattalionLeader))
		return ent;
	...
	if (!Engine.QueryInterface(leader, IID_BattalionLeader))
		return INVALID_ENTITY;

	return leader;
}

function CollapseBattalionEntities(ents)
{
	const collapsed = [];
	const seen = {};
	...
	for (const ent of ents)
	{
		...
		const commandEnt = leader != INVALID_ENTITY ? leader : ent;

		if (seen[commandEnt])
			continue;

		seen[commandEnt] = true;
		collapsed.push(commandEnt);
	}
    ...
    forceTemplate)
{
	...
	if (hasBattalion)
	{
		ents = CollapseBattalionEntities(ents);
	...
	for (const ent of ents)
	{
		...
		const cmpBattalionLeader =
			Engine.QueryInterface(
				ent,
				IID_BattalionLeader);

		const cmpBattalionMember =
			Engine.QueryInterface(
				ent,
				IID_BattalionMember);

		if (cmpBattalionLeader || cmpBattalionMember)
		{
			nonformedUnitAIs.push(
				cmpUnitAI);
		...
		const nullFormation =
			(formationTemplate ||
			cmpUnitAI.GetFormationTemplate())
			== NULL_FORMATION;

		if (
			nullFormation ||
			!cmpUnitAI.CanUseFormation(
				formationTemplate ||
				NULL_FORMATION)
		)
		{
